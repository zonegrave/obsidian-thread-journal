...
在主 thread 或工作区运行 **编辑 checkpoint 模板**，可以为它单独增删、排序和配置字段。启用 Modal Form 时，模板窗口显示紧凑字段列表，眼睛按钮直接控制字段是否用于新 checkpoint，“添加字段”和“编辑”会调用 Modal Form；未启用时使用内置字段编辑器。独立模板保存在主 thread 的 `checkpoint_fields` 属性中；选择“使用全局默认模板”会删除该属性，重新继承 **设置 → Thread Journal → 默认 Checkpoint 模板**。
...
每张带块 ID 的 checkpoint 卡片都有“编辑”按钮。编辑表单只载入该条记录实际保存的字段，包括后来废弃或已从模板删除的字段；保存后按原块 ID 原位替换，不会新增重复记录。编辑历史 checkpoint 不会重新触发 `status_after` 状态流转。
