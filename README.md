...
**管理已打开的 thread** 用于管理当前 Obsidian 窗口里已经打开的 thread：

- Context 和 Workspace 按 `thread_id` 合并为一个候选项，不会因为两个界面同时打开而重复出现。
- 列表显示 thread 层级、状态以及当前已打开的界面，并支持模糊搜索。
- 每个候选项可直接跳转到 Context 或 Workspace；目标已有标签时直接聚焦，未打开时在当前标签组新建，尚无工作区时按需创建。
- **关闭标签** 会关闭该 `thread_id` 的全部 Context 与 Workspace 标签，包括重复标签；不修改 thread 状态，也不删除文件。
- 最近使用的 thread 排在前面；最近使用顺序仅保留在本次插件运行期间。

这个命令负责跨 thread 跳转与清理标签；下面的 **切换 thread 与工作区** 则负责在当前 thread 的两个界面之间往返。
...
- 最左侧 thread 图标表示虚拟的 Thread 根节点；没有 parent 的 thread 是它的直接子节点，点击图标后的分隔箭头可在悬浮菜单中切换。中间显示完整祖先路径和当前 thread，每段均可跳转；点击某一级之后的分隔箭头会显示它的直接子 thread，当前 thread 有子节点时末尾也会显示箭头。正文上方的工具条优先向下展开菜单，正文下方的工具条向上展开；空间不足时自动换向。
- 右侧双向切换按钮在主 thread 中打开按 `thread_id` 配对的工作区，在工作区中返回主 thread；旁边的切换器只列当前窗口中已经打开的 thread，并按 `thread_id` 合并主文件与工作区，选择后回到最近使用的界面。
- 最右侧可在“投入中”和“全部”之间临时切换；这个范围只控制左侧根节点和层级分隔菜单。默认范围和工具条位于正文上方／下方可在设置中修改；位于下方时会自动避让 Obsidian 右下角状态栏，按钮提示向上显示。顶部工具条的提示向下显示。
...
| **管理已打开的 thread** | 任意位置 | 跳转到指定界面，或关闭一个 thread 的全部已打开标签 |
...
- **Breadcrumb 层级默认范围**：根节点和分隔菜单默认显示投入中的 `active` 与 `dormant`，或显示全部状态；工具条最右侧可临时切换。
