...
在主 thread 或工作区运行 **编辑 checkpoint 模板**，可以为它单独增删、排序和配置字段。启用 Modal Form 时，模板窗口显示紧凑字段列表，废弃开关直接位于列表行中，“添加字段”和“编辑”会调用 Modal Form；未启用时使用内置字段编辑器。独立模板保存在主 thread 的 `checkpoint_fields` 属性中；选择“使用全局默认模板”会删除该属性，重新继承 **设置 → Thread Journal → 默认 Checkpoint 模板**。
