...
```markdown
> [!thread-log]
> (thread_log:: 2026-09-13T14:35:27)
>
> 完成了第一轮接口验证
> 还有一项需要继续调研。

^log-20260913-143527-a1b2c
```

- 正文是自由的多行 Markdown，可以使用双链、列表等；查询会保留整个正文。
- 完整时间戳只存于 `thread_log` 字段，用于筛选和排序；渲染时生成紧凑时间，隐藏原始字段。
- 块 ID 单独放在 callout 末尾，用于从查询卡片精确定位原始记录。
